...
void Renderer::OnResize(uint32_t width, uint32_t height)
{
	...
	delete[] m_AccumulaionData;
	m_AccumulaionData = new glm::vec4[width * height];
...
{	
	...
	if (m_FrameIndex == 1)
		memset(m_AccumulaionData, 0, m_FinalImage->GetWidth()*m_FinalImage->GetHeight()*sizeof(glm::vec4));
	...
	std::for_each(std::execution::par, m_ImageVerticalIterator.begin(), m_ImageVerticalIterator.end(), [this](uint32_t y) {
		std::for_each(std::execution::par, m_ImageHorizontalIterator.begin(), m_ImageHorizontalIterator.end(), [this, y](uint32_t x) {
			...
			m_AccumulaionData[x + y * m_FinalImage->GetWidth()] += color;

			glm::vec4 accumulatedColor = m_AccumulaionData[x + y * m_FinalImage->GetWidth()];
